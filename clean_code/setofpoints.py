...
class SetOfPoints:
	...
	def closest_points(self, P: npt.NDArray, N: int) -> "SetOfPoints":
		"""
		Return a subset of the N points closest to a given query point P.

		Args:
			P (np.ndarray): A point in d-dimensional space of shape (d,).
			N (int): Number of closest points to return.

		Returns:
			SetOfPoints: New SetOfPoints containing the N closest points
			and their corresponding normalized weights.

		Raises:
			ValueError: If N is greater than the total number of points
						or if P has the wrong dimensionality.

		Example:
			>>> points = np.random.rand(100, 3)
			>>> point_set = SetOfPoints(points)
			>>> query = np.array([0.5, 0.5, 0.5])
			>>> closest = point_set.closest_points(query, 5)
			>>> print(len(closest))  # 5
		"""
		
		if P.shape[0] != self.dimension():
			raise ValueError(f"Query point dimensionality {P.shape[0]} "
							 f"does not match point set dimension {self.dimension()}.")

		if N > len(self):
			raise ValueError("Requested more points than available in the set.")

		# Compute squared Euclidean distances for efficiency
		distances = np.linalg.norm(self.points - P, axis=1)
		nearest_indices = np.argsort(distances)[:N]

		return self.subset(nearest_indices)
